...
const hotel = { id: "expense-1", tripId: trip.id, name: "Hotel", plannedCostId: null, category: "Accommodation", amount: 120, expenseDate: "2026-09-01", createdAtUtc: "2026-01-01T00:00:00Z" };
...
describe("ExpenseTracking", () => {
    ...
    it("shows the loaded expense and recalculates the remaining budget", async () => {
        render(<ExpenseTracking trip={trip} refreshKey={0} onFormOpenChange={vi.fn()} />);
        expect(await screen.findByText("Hotel")).toBeTruthy();
        expect(screen.getAllByText("€120").length).toBeGreaterThan(0);
        expect(screen.getByText("Actual remaining").parentElement?.textContent).toContain("€380");
    });
