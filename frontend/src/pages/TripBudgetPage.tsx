...
/** Shows and manages the recorded expenses for the current trip. */
export function TripBudgetPage() {
  ...
  // Tell the workspace to warn before tab navigation while a form is open.
  useEffect(() => {
        setHasUnsavedForm(isAddingBudgetItem || editingBudgetItemId !== null);
        return () => setHasUnsavedForm(false);
  }, [editingBudgetItemId, isAddingBudgetItem, setHasUnsavedForm]);
  
  // Reload expenses whenever the workspace changes to a different trip.
  useEffect(() => {
  ...
  const createBudgetItem = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        `${apiBaseUrl}/api/trips/${trip.id}/budget-items`,
        {
          method: "POST",
      ...
      setBudgetItemFormError("Could not save this expense. Please try again.");
    } finally {
  ...
  const saveBudgetItem = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        `${apiBaseUrl}/api/trips/${trip.id}/budget-items/${editingBudgetItemId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      setBudgetItemFormError("Could not save these changes. Please try again.");
    } finally {
      setIsSavingBudgetItem(false);
    }
  };
  ...
  const deleteBudgetItem = async (item: BudgetItem) => {
    ...
    try {
      const response = await fetch(
        `${apiBaseUrl}/api/trips/${trip.id}/budget-items/${item.id}`,
        { method: "DELETE" },
      );
      ...
      setBudgetItemsError("Could not delete this expense. Please try again.");
    } finally {
  ...
  return (
    <section className="detail-section trip-budget-page">
        ...
        <form className="budget-item-form" onSubmit={createBudgetItem}>
          <label>
            ...
            <input
              ...
              onChange={(event) => updateBudgetItem("name", event.target.value)}
                ...
                <form className="budget-item-form" onSubmit={saveBudgetItem}>
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateEditingBudgetItem("name", event.target.value)
                      }
                      required
                    />
                  </label>
                  <div className="form-row">
                    ...
                    <label>
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          updateEditingBudgetItem("amount", event.target.value)
                        }
                        required
                      />
                    </label>
