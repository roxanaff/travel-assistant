...
import type { ItineraryItem } from "../../../src/types/itineraryItem";
...
describe("getTripDays", () => {
    it("returns every inclusive date in the trip", () => {
        expect(getTripDays(trip)).toEqual([
            "2026-09-01",
            "2026-09-02",
            "2026-09-03",
        ]);
...
describe("itinerary ordering", () => {
    it("puts timed entries first and orders them by start time", () => {
        const sorted = sortDatedItems([
            item({ name: "Untimed" }),
            item({ name: "Later", startTime: "14:00" }),
            item({ name: "Earlier", startTime: "09:00" }),
        ]);
    ...
    it("orders unscheduled ideas by priority", () => {
        const sorted = sortUnscheduledItems([
            item({ name: "Optional" }),
            item({ name: "Must do", priority: "MustDo" }),
            item({ name: "Would like", priority: "WouldLikeToDo" }),
        ]);
        expect(sorted.map((entry) => entry.name)).toEqual([
            "Must do",
            "Would like",
...
describe("getOpeningHoursWarning", () => {
    it("warns when a scheduled activity would end after closing", () => {
        ...
        ).toBe("This activity ends after the entered closing time.");
    });

    it("warns about an untimed duration when it starts within an hour of closing", () => {
        expect(
            getOpeningHoursWarning(
                item({ startTime: "17:30", closingTime: "18:00" }),
            ),
        ).toBe("This activity starts within one hour of closing.");
    });
